...
WisdomAI supports connecting to Microsoft SharePoint as an unstructured data source. The SharePoint connector crawls files from your SharePoint Online sites and document libraries, making them available for AI-powered search and analysis. This connector enables multi-site ingestion across diverse file formats including documents, PDFs, spreadsheets, and images, while providing granular filtering by document library or folder path and incremental synchronization to ensure only new or modified content is processed after the initial crawl.
...
* **Authentication Methods:** You must have one of the authentication methods below configured:
...
6.  **Configure SharePoint Sites:**
    ...
    * (Optional) Click **Add Library** to restrict crawling to specific document libraries. If left blank, all libraries in the site are crawled.

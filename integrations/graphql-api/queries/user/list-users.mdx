...
The `listUsers` query returns users in a workspace along with their attributes and role assignments. Use it to look up user IDs before calling [impersonateUser](/integrations/graphql-api/queries/auth/impersonate-user) or [setUserAttributes](/integrations/graphql-api/mutations/user/set-user-attributes).
...
<ParamField path="userAttributes" type="[UserAttribute!]">
  ...
  See [User attributes](/integrations/embedding/user-attributes) for how these sources differ.
</ParamField>

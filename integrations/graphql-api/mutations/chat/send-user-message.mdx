...
| `query` | `DeltaInput` | Required |
| `createHiddenConversation` | `Boolean` | Optional — default: `false` |
| `toolSelection` | `ToolSelection` | Optional |
| `selectedModelName` | `String` | Optional |
| `isUserOnboarding` | `Boolean` | Optional — default: `false` |
| `editArtifactMode` | `Boolean` | Optional — default: `false` |
| `chatThinkingEffort` | `ChatThinkingEffort` | Optional |
| `disableClarifications` | `Boolean` | Optional — default: `false` |
| `enableDeepAnalysis` | `Boolean` | Optional — default: `false` |
| `isEvaluatingTrigger` | `Boolean` | Optional — default: `false` |
...
See [ResponseStatus](/integrations/graphql-api/objects/response-status) for the full schema.
...
| 400 | Bad Request | Missing required field or malformed `DeltaInput` |
| 401 | Unauthorized | Missing or expired JWT |
| 403 | Forbidden | User lacks read access to the specified domain |
| 404 | Not Found | `conversationId` or `domainId` does not exist |

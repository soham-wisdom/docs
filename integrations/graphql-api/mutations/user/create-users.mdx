...
| Field | Type | Required |
|-------|------|----------|
| `input` | `CreateUsersInput` | Required |
...
| `workspaceID` | `ID` | Optional — required for multi-workspace deployments |
| `roleAssignments` | `[RoleAssignmentInput!]` | Optional |
| `sendWelcomeEmail` | `Boolean` | Optional — default: `false` |
| `userAttributes` | `[UserAttributeInput!]` | Optional |
| `userGroupIds` | `[ID!]` | Optional |
...
|-------|------|-------------|
| `status.code` | `String` | `"OK"` on success |
| `status.message` | `String` | Human-readable status description |

<Info>
  `status.code` / `status.message` come from the `ResponseStatus` type — distinct from the `UserStatus` enum (e.g., `ACTIVE`) on the `User` object.
</Info>
...
| Code | Status | Meaning |
|------|--------|---------|
| 200 | OK | Users created (or role assignments updated) successfully |
| 400 | Bad Request | Invalid email format or missing required fields |
| 401 | Unauthorized | Missing or expired access token |
| 403 | Forbidden | Caller lacks permission to provision users |

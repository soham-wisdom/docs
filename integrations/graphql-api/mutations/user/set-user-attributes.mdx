...
<Warning>
  This mutation **replaces all DATABASE-sourced attributes** on the user. Any attributes not included in the new list will be removed. JWT-sourced attributes (from SSO claims) are not affected. See [User attributes](/integrations/embedding/user-attributes) for the difference between attribute sources.
</Warning>

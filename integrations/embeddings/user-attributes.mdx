...
When the same key exists in both sources, the `DATABASE` value takes precedence — unless a transient attribute is passed at impersonation time, which overrides both for that session only.

## Setting attributes

Attributes can be set at three points in a user's lifecycle: when the user is first provisioned, updated later via API, or passed transiently at impersonation time.

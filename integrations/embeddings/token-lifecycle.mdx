...
iframe.src = `https://{ACCOUNT}.askwisdom.ai/embed/search?token=${newJWT}`;
```
...
When an expired or invalid JWT is passed in the embed URL, WisdomAI redirects the iframe to the login page. To prevent this, implement the `postMessage` refresh listener described above so that tokens are refreshed before expiry.

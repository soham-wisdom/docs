...
The `<Dashboard>` component renders an entire Wisdom dashboard — header, filter bar, and widget grid:
...
When you want full control over the card _around_ a widget — your own container, header, spacing, menus, and empty/loading states — drop down to the composable layer. Instead of Wisdom's pre-built widget card, you render just the visualization and wrap it in your own markup.
...
**2. Decoupled filters — place the filter UI anywhere.** To move the filter controls out of the dashboard and into your own layout (a sidebar, a toolbar, a drawer), compose the pieces under a single `DashboardProvider`. The filters and the widgets share that provider's state, so controls placed anywhere drive the same dashboard:

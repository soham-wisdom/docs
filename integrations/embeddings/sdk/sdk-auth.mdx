...
Construct a `WisdomAI` client with your access token and base URL, then call `getAuthToken()` to mint a short-lived JWT. It returns `{ jwt, baseUrl }` — exactly the shape the frontend provider expects.
...
See _Quickstart_ for the full Express endpoint example.

...
Use [`impersonateUser`](/integrations/embeddings/graphql-api/queries/auth/impersonate-user) to generate a short-lived JWT for a user. Pass this JWT as the `token` query parameter in your iframe embed URL.
...
WisdomAI will request a new token automatically via `postMessage` before expiry. See [Impersonate user](/integrations/embeddings/graphql-api/queries/auth/impersonate-user) for the full refresh flow.

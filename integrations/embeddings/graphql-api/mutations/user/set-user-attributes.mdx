...
<Warning>
  This mutation **replaces all DATABASE-sourced attributes** on the user. Any attributes not included in the new list will be removed. JWT-sourced attributes (from SSO claims) are not affected. See [User attributes](/integrations/embeddings/user-attributes) for the difference between attribute sources.
</Warning>
...
<ParamField path="userId" type="ID!" required>
  The ID of the user whose attributes should be updated. Use [listUsers](/integrations/embeddings/graphql-api/queries/user/list-users) to find user IDs.
</ParamField>
...
<Tip>
  For attributes that are stable (e.g., `account_id`), set them once at creation via `userAttributes` in [createUsers](/integrations/embeddings/graphql-api/mutations/user/create-users). For session-scoped overrides that should not be persisted, use `attributes` in [impersonateUser](/integrations/embeddings/graphql-api/mutations/auth/impersonate-user) instead.
</Tip>

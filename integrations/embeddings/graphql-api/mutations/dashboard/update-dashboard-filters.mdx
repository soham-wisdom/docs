...
<ParamField path="filterMutations" type="FilterMutationInput!" required>
  Filter mutation configuration specifying the changes to apply. See [FilterMutationInput](/integrations/embeddings/graphql-api/objects/filter-mutation-input).
</ParamField>

## Response

Returns the updated `Dashboard` object with refreshed filter definitions. See [Dashboard](/integrations/embeddings/graphql-api/objects/dashboard) for the schema.
...
<CardGroup cols={2}>
  ...
  <Card title="DashboardFilterDefinition Object" icon="filter" href="/integrations/embeddings/graphql-api/objects/dashboard-filter-definition">

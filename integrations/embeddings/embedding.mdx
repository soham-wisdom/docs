...
<Steps>
  ...
  <Step title="Use the JWT in the embed URL" titleSize="h3">
    Append the JWT as a `token` query parameter to the embed URL and set it as the `src` of your embed. Ensure the `allow` attribute includes `microphone; camera`.\
    \
    The following examples use an `<iframe>`, but the same URL structure applies regardless of your integration method.

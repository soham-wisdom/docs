...
<Warning>
  Currently, MCP endpoints work with **the account's actual domain**, the same domain you use to log in. Both `https://{ACCOUNT}.wisdom.ai/mcp` and `https://{ACCOUNT}.askwisdom.ai/mcp` are valid depending on your deployment.
</Warning>
...
| Tool | Description |
| :-- | :-- |
| [**chat**](/integrations/embeddings/mcp-server/tools/chat) | Ask natural-language questions about your data domains. Returns text, tables, and chart visualizations. |
| [**list-domains**](/integrations/embeddings/mcp-server/tools/list-domains) | Discover available domains and their IDs before making queries. |
| [**echo**](/integrations/embeddings/mcp-server/tools/echo) | Test connectivity and verify the request/response flow is working. |

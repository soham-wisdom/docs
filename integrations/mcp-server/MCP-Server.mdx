...
keywords: ["MCP", "MCP server", "Model Context Protocol", "Wisdom MCP server"]
boost: 15
---
...
| [**list_domains**](/integrations/mcp-server/tools/list-domains) | Discover available domains and their IDs before making queries. |
...
5. Select a tool (for example, **list_domains**) and click **Call** to test it.
...
| Empty responses from chat | Make sure you are providing a valid `domainId`. Use the **list_domains** tool first to find available domain IDs. |

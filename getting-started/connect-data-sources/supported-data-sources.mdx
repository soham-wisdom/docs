...
boost: 0.15
og:description: "This page covers the data sources that can be connected to WisdomAI, including databases, data warehouses, and Files (PDFs, PPTs, Docs). It outlines the"
---
...
To learn how to connect to your Data Sources, read the [Quickstart: Connect and Test](/setting-up-wisdom-ai/basic-tutorial-connect-and-test).
...
<Tabs>
  ...
  <Tab title="GitLab">
    The following table lists the requirements you must provide when connecting to GitLab.

    | Requirement | Description |
    | :-- | :-- |
    | Connection name | A name to identify this connection |
    | Project URL | The URL of the GitLab project you want WisdomAI to extract context from |
    | Access token | Required for private projects |
    | Branch | The branch WisdomAI should crawl |

    <Note>
      An access token is required for private projects.
    </Note>

    Here's an example connection:

    ```txt
    Connection name: product-docs-project
    Project URL: https://gitlab.com/your-group/your-project
    Access token: ********
    Branch: main
    ```
  </Tab>
  <Tab title="Azure DevOps">
    The following table lists the requirements you must provide when connecting to Azure DevOps.

    | Requirement | Description |
    | :-- | :-- |
    | Connection name | A name to identify this connection |
    | Repository URL | The URL of the Azure DevOps repository you want WisdomAI to extract context from |
    | Organization | The Azure DevOps organization that owns the repository |
    | Project | The Azure DevOps project that contains the repository |
    | Repository | The name of the repository |
    | Repository ID | The unique identifier for the repository |
    | Server base URL | The base URL for on-premises Azure DevOps Server instances |
    | Branch | The branch WisdomAI should crawl |

    You must also choose an authentication method:

    | Authentication method | Description |
    | :-- | :-- |
    | Connect as user | Authenticate with your own Azure DevOps user account via OAuth |
    | Wisdom managed app | Use WisdomAI's managed OAuth app credentials |
    | Service principal | Authenticate using a Tenant ID, OAuth client ID, and OAuth client secret |
    | Managed identity | Authenticate using an Azure managed identity |
    | Personal access token | Authenticate using a PAT generated in Azure DevOps |
    | SSH key | Authenticate using an SSH key pair |

    <Note>
      The **Tenant ID**, **OAuth client ID**, and **OAuth client secret** fields are only required when using the **Service principal** authentication method.
    </Note>
    ...
    ```txt
    Connection name: product-docs-repo
    Repository URL: https://dev.azure.com/your-org/your-project/_git/your-repo
  ...
  <Tab title="Dremio Connection">
    The following table lists the requirements you must provide when connecting to Dremio.

    | Requirement | Description |
    | :-- | :-- |
    | Connection name | A name to identify this connection |
    | Host | The hostname or IP address of your Dremio server |
    | Port | The port number used for the connection |
    | Authentication | Basic (Software / on-prem) or Bearer (Cloud / PAT) |
    | Username | The Dremio username (Basic authentication) |
    | Password/PAT | The password or personal access token, depending on the authentication method |
    | Use TLS | Whether to encrypt the connection using TLS |
    | Skip TLS Certificate Verification | Whether to skip verification of the TLS certificate |
    | Catalog Filters | Optionally filter Dremio catalogs (sources / spaces) and their schemas |

    <Note>
      Each catalog filter can have its own schema filters. You can also add alternate credential sets under **Advanced** to use different credentials based on user attributes (e.g. region, environment); any fields left empty for an alternate are inherited from the primary connection at runtime.
    </Note>

    Here's an example connection:

    ```txt
    Connection name: analytics-dremio
    Host: dremio.example.com
    Port: 9047
    Authentication: Bearer (Cloud / PAT)
    Password/PAT: ********
    Use TLS: True

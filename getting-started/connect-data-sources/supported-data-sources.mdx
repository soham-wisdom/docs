...
Connect any server implementing the [Model Context Protocol](https://modelcontextprotocol.io).

### MCP Server Requirements

The Model Context Protocol (MCP) allows WisdomAI to fetch real-time data and scan metadata. To connect, you need:

| Requirement            | Description                                     |
| :--------------------- | :---------------------------------------------- |
| **Transport Type**     | Streamable HTTP, SSE, or STDIO.                 |
| **Server URL/Command** | The endpoint or command used to run the server. |
| **Auth Type**          | API Key, OAuth, or None.                        |

<Info>
  For detailed setup instructions, see [Connect an MCP Server](/setting-up-wisdom-ai/connect-mcp-server).
</Info>
...
<CardGroup cols={3}>
  <Card title="Connect Unstructured Repositories" icon="folder-plus" href="/improve-wisdom-ai-responses/connect-unstructed-repositories-datasets">
    Integrate unstructured data sources like documents or knowledge bases to enrich your analysis.
  </Card>

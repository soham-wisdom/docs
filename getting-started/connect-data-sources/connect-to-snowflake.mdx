...
## Prerequisites
Before connecting WisdomAl to Snowflake, ensure you have:
...
* **Account**: Your Snowflake account identifier. This consists of the name of the account prefixed by its organization (e.g., myorg-account123). Refer to Snowflake documentation for details on finding your account identifier.
...
* **Private Key**: The private key associated with the Snowflake user. Ensure the key is in PEM format, including the `----BEGIN...PRIVATE KEY` and `-END...PRIVATE KEY` headers and footers. Refer to Snowflake documentation for details on generating a private key.
...
* **[Optional] Allowlist**: You may need to allowlist WisdomAl's CIDR block in your Snowflake network policies or firewalls to allow incoming connections. The CIDR block is `35.238.115.103/32` if your instance was created before July 24, 2025 and `34.82.248.105/32` if your instance was created on or after July 24, 2025.
...
See Snowflake documentation for more details on granting privileges to your role.
...
3.  Fill in the connection details in the "Snowflake connection details" section:
    ...
    * **User**: Enter the Snowflake username (e.g., user).
...
* Verify that the Snowflake role assigned to WisdomAl has all the necessary USAGE and SELECT privileges as detailed in the "Using a Custom Role" section.

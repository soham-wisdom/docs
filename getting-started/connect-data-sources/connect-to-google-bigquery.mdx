...
3. Under **Account Information**, paste the contents of your service account JSON key into **Service Account Info JSON**.
   > This field is required for crawling and metadata discovery.
4. (Optional) Enter **Authorization scopes** to restrict the OAuth permission scopes granted to the connection.
5. (Optional) Enter a **Billing Project ID** to specify which project to bill for queries. Leave this field empty to use your service account's default project.
6. Under **Projects**, configure at least one BigQuery project:
   1. In the **Project 1** card, enter your Google Cloud **Project ID**.
   2. (Optional) Enter **Dataset filters** to restrict which datasets WisdomAI crawls for that project.
   3. To connect additional projects, click **\+ Add Project** and repeat these sub-steps for each one.
...
<Frame>
  ![Oauth Step8](/images/Oauth-step8.png)
</Frame>

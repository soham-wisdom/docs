...
In some complex cases, the AI might not follow a SQL example perfectly, leading to logical errors like double counting. You can prevent this by supplementing your Reviewed Queries with explicit instructions in the Language Context.
...
- [cite_start]**Use Common Table Expressions (CTEs) for clarity.** For complex logic, breaking the query into understandable CTEs makes it more readable and allows for reuse in future metrics[cite: 357, 422].
- [cite_start]**Maintain consistent structures for related metrics.** When creating multiple variations of a metric, use a shared CTE to define the base calculation[cite: 358, 360]. [cite_start]This prevents logic from drifting between different reports[cite: 461].

### Combine Context Types for Greater Accuracy

[cite_start]In some complex cases, the AI might not follow a SQL example perfectly, leading to logical errors like double counting[cite: 463, 464]. [cite_start]You can prevent this by supplementing your queries with explicit instructions in the Language Context[cite: 463].

[cite_start]For example, add a rule like: `"Never sum up country_population column across different rows for the same country"` to prevent incorrect aggregations[cite: 465].

## Test and Measure Impact with Evaluation Sets

Context modeling is an iterative process. [cite_start]The best way to validate your work is to create a test set of natural language questions and SQL pairs[cite: 467]. [cite_start]As you add more knowledge and context, you can track the accuracy score of the evaluation to see it go up[cite: 468].

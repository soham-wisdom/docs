...
- **What you'll do**: Document tables and columns, define business entities, establish relationships, and add simple rules in natural language.
...
## Step 2: Define Complex Metrics

For critical business calculations that require specific or complex logic, you need to provide the AI with pre-vetted examples. This ensures that common or important questions are always answered with guaranteed accuracy and consistency.
...
- **Use Common Table Expressions (CTEs) for clarity.** For complex logic, breaking the query into understandable CTEs makes it more readable and allows for reuse in future metrics[cite: 422].
...
In some complex cases, the AI might not follow a SQL example perfectly, leading to logical errors like double counting. You can prevent this by supplementing your Reviewed Queries with explicit instructions in the Language Context[cite: 463].

For example, add a rule like: `"Never sum up country_population column across different rows for the same country"` to prevent incorrect aggregations.

...
keywords: ["agent", "agents", "nodes"]
boost: 0.05
og:description: "Nodes are the building blocks of a Visual Mode agent. Each node is a modular instruction that performs one focused task: fetching data, running analysis"
---
...
<AccordionGroup>
  ...
  <Accordion title="Web Search">
    Searches the web and brings results into the workflow as context. Use this node when your agent needs information from outside your connected data sources: recent news, competitor activity, market events, or any external signal that your internal data does not capture.
  ...
  <Accordion title="MCP Node">
    ...
    **When to use:** Any workflow that needs to act on an external system rather than just read from your connected data sources.
...
<CardGroup cols={2}>
  <Card title="Build an Agent in Visual Mode" icon="diagram-project" href="/setting-up-wisdom-ai/agents/visual-mode">
    Step-by-step guide to building your first Visual Mode agent.
  </Card>

  <Card title="Agent Best Practices" icon="star" href="/setting-up-wisdom-ai/agents/agent-best-practices">

---
title: "Advanced Tutorial: Creating Knowledge"
---

This tutorial, intended for Admin users and Data Administrators, details the three primary methods for creating and curating knowledge to improve AI-powered analytics. The learning objectives are to:

- [Model data using a formal semantic layer.](#formal-semantic-modeling)
- [Enrich the AI's understanding with natural language.](#natural-language-context)
- [Improve accuracy through direct user feedback.](#feedback-based-context)

To follow this tutorial, you should:

- Have Admin/Data Admin access.
- Have completed the Basic Tutorial to connect data sources and create domains.
- Have at least one domain already created.
...
<Steps>
  <Step title="Table and Column Documentation">
    Properly documenting tables and columns provides foundational context to enable effective analytics. For each table, provide a clear, plain-English description of what the table represents, and include a statement of what each row's granularity represents, such as, "Each row represents a single sales transaction from a point-of-sale system". For each column, define a user-friendly Display Name and a Description in business terms.

    | Field        | Purpose                                                         |
    | ------------ | --------------------------------------------------------------- |
    | Display Name | Sets a user-friendly name for a column.                         |
    | Description  | Explains what the table or column represents in business terms. |
  </Step>
  <Step title="Defining Entities">
    Group columns into logical business objects that represent real-world concepts, like "Customer," "Product," or "Order". When creating an entity, you will name it, map it to one or more data source tables, and configure its properties, including its primary key and the default display name field.
  </Step>
...
<Steps>
  <Step title="Manual Context Entry">
    ...
    <Note>
      Refer to the Connect and Experiment Use Case to see an example of the impact of adding context.

      To learn how to add Context, read the article 
    </Note>
  </Step>
...
<Steps>
  ...
  <Step title="Advanced Feedback and Corrections">
    ...
        \* SQL Editing for Power Users: Each answer in the chat has an "Edit" button, which allows power users to modify the underlying SQL query directly to fix calculation issues, add joins, or modify filters. Edited queries become part of the system's knowledge base for handling similar questions in the future.

        \* Negative Feedback Workflow: Negative feedback is channeled to administrators for review. This is managed through a "Questions" tab in the interface, visual indicators that highlight chats with negative feedback, and automatic email notifications that are sent to administrators for prompt attention.
  </Step>
</Steps>

...
<Steps>
  ...
  <Step title="Manage subscriptions and templates" icon="users-gear">
    ...
    - Row-level security (RLS) and parameterized data source connections are enforced per subscriber, the creator's permissions are never used for a subscriber's run.
...
<AccordionGroup>
  ...
  <Accordion title="Is scheduled fan-out supported?">
    Not in the current release. V1 ships manual triggers only.
  </Accordion>

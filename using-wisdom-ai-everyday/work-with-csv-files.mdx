...
**Best for:** Making a dataset available to your entire team.

Admins can add or replace CSV files as tables within a shared domain or replace exiting tables. This change affects the domain-level schema, meaning every Explorer assigned to that domain can query this data.
...
Admins or Explorers can create their own domains by uploading a file. These domains are private by default; no other user can see them unless the creator explicitly shares them.
...
4. In the **Upload CSV or TSV file** modal that opens, leave the default option **New Domain** selected and continue uploading your CSV files.

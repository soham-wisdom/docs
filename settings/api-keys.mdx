...
See the [GraphQL API](/integrations/graphql-api/GraphQL-API) and [WisdomAI MCP Server](/integrations/mcp-server/MCP-Server) documentation for full request examples.
...
Your server uses an impersonation key as the access token for the [`impersonateUser`](/integrations/graphql-api/queries/auth/impersonate-user) query, which returns a short-lived JWT for a specific user. You then pass that JWT into the WisdomAI iframe. See [Impersonate User](/integrations/graphql-api/queries/auth/impersonate-user) for the full embedded session flow.
...
<CardGroup cols={2}>
  ...
  <Card title="WisdomAI MCP Server" icon="plug" href="/integrations/mcp-server/MCP-Server">

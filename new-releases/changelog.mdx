...
<Update label="Spring 2026">
  ...
  Agent edits no longer modify the live version in place. Every change creates a draft, leaving the published version running undisturbed until you're ready. Publishing a new version increments the version number and locks the published version. Run logs are permanently tied to the version that executed them, so reviewing a past run always shows the correct graph and configuration.
  ...
  <Accordion title="FAQs">
    <AccordionGroup>
      ...
      <Accordion title="Is there a row limit on queries executed against Dremio?">
        Wisdom applies its standard query result limits. Dremio's own resource controls also apply.
      </Accordion>
    </AccordionGroup>
  </Accordion>
  ...
  **⚙️ How it works:**
  ...
  <Note>
    The Knowledge Suggestions panel is currently in Alpha while quality tuning is ongoing. Review and edit suggestions before accepting.
  </Note>

  <Accordion title="FAQs">
    <AccordionGroup>
      <Accordion title="Who can accept knowledge suggestions?">
        Only users with write access to the domain. Explorers and analysts can view the "Context Used" explanation on any answer but cannot manage knowledge entries.
      </Accordion>

      <Accordion title="Does accepting a suggestion take effect immediately?">
        Yes. The entry is written to the domain knowledge base and available for retrieval on the next query.
      </Accordion>
    </AccordionGroup>
  </Accordion>

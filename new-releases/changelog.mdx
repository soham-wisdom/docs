...
<Update label="Spring 2026">
  ...
  Agent edits no longer modify the live version in place. Every change creates a draft, leaving the published version running undisturbed until you're ready. Publishing locks the draft as the new version and increments the version number. Run logs are permanently tied to the version that executed them, so reviewing a past run always shows the correct graph and configuration.

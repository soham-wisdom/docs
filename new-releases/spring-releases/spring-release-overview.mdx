---
title: "Release Overview"
icon: "rocket-launch"
tag: "new"
---

Explore the latest features in WisdomAI's **Spring 2026** launch, focused on agent reliability, automated domain intelligence, and deeper embeddability.

<CardGroup cols={2}>
  <Card title="Parameterized Agents" icon="sliders" href="/new-releases/spring-releases/parameterized-agents">
    Run one agent for multiple users or inputs, with each run scoped to its recipient's data permissions and parameters.
  </Card>

  <Card title="Continuous Context Extraction" icon="rotate" href="/new-releases/spring-releases/continuous-context-extraction">
    Connect your domain directly to GitHub and dbt repositories and keep your knowledge layer synchronized on a schedule.
  </Card>

  <Card title="Dashboard Versioning" icon="clock-rotate-left" href="/new-releases/spring-releases/dashboard-versioning">
    Every dashboard change creates a draft. Publish when ready and revert to any prior version at any time.
  </Card>

  <Card title="Domain Health" icon="heart-pulse" href="/new-releases/spring-releases/domain-health">
    A scored readiness signal across schema, context, and usage, with a prioritized list of actionable improvements.
  </Card>
</CardGroup>

<Card title="Embedded Analytics SDK" icon="code" href="/new-releases/spring-releases/embedded-sdk-updates">
  A React component library for embedding WisdomAI dashboards natively inside your product, with full theming and host-controlled layout.
</Card>

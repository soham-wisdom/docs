...
Every agent has a **Reports** column on the main **Agents** page, separate from **Run logs**. Reports is where you find and export the artifacts an agent has generated, regardless of how (or whether) it was delivered.
...
<Frame>
  ![Agent Reports Column](/images/agent-reports-column.png)
</Frame>

<Frame>
  ![An agent's Reports list](/images/agent-reports-list.png)
</Frame>

<Note>
  Reports is separate from delivery. The Artifact Builder node still formats and sends each run's output to Email or Slack — see [Configure delivery](/advanced-features/agents/tutorial-create-agent#step-5-configure-delivery). Reports is where you go afterward to find, revisit, or export any artifact your agent has produced, whether or not a notification went out for it.
</Note>

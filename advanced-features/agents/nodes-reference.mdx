...
Data nodes bring information into the workflow. They are typically the first nodes connected after the trigger, and their output, a structured dataframe or freeform text, flows into downstream Reasoning nodes.
...
Reasoning nodes operate on dataframes and can take text/free form imputs from other nodes to produce AI analysis. They perform analysis, summarization, and derivation to transform raw data into insights. 
...
Takes a dataframe or context as input and produces a textual narrative. Unlike Analysis, which outputs structured data, Summary outputs prose: a paragraph or set of sentences generating summary from inputs.
...
Evaluates a condition against the data in the workflow and routes execution down one of two or more branches — the IF branch if the condition is true, the ELSE branch if it is not. Nodes on the inactive branch receive a no-op signal and are skipped entirely — they neither execute nor produce output, but simply relay the inputs ahead.
...
**Important:** If a downstream node uses input conditionally, wire it through the If/Else node instead of using a direct edge. A direct edge bypasses the conditioning and always passes the data.
...
Ingests dataframes and text produced during the workflow run and formats them into a structured report. The Artifact Builder does not infer formatting — you define the structure explicitly in its instructions.
...
**Supported output formats:** Email with full table rows and fluid layout and Slack. PDF, DOCX, PPTX, and XLSX are coming soon.
